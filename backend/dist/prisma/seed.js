            ...
            data: {
                ...
                cleanerProfile: {
                    create: {
                        ...
                        completedJobsCount: Math.floor(Math.random() * 30) + (i <= 3 ? 15 : 5),
                        freeJobsUsed: Math.min(20, Math.floor(Math.random() * 25)),
                    },
                },
                wallet: {
                    create: {
                        balanceMad: i <= 4 ? Math.random() * 500 + 100 : Math.random() * 50 - 100,
                    },
                },
                debtThresholds: {
                    create: {
        ...
        for (const service of services.slice(0, Math.floor(Math.random() * 3) + 1)) {
            const cleanerService = await prisma.cleanerService.create({
                data: {
                    ...
                    priceMad: Math.floor(Math.random() * 200) + 80,
                    active: true,
                },
            });
    ...
    for (let i = 0; i < 5; i++) {
        ...
        await prisma.supportTicket.create({
            data: {
                ...
                channel: ['EMAIL', 'FORM', 'WHATSAPP'][Math.floor(Math.random() * 3)],
